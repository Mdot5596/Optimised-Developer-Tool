...
uniform sampler2D  sodaCanTex;  
vec4 texColor = texture( sodaCanTex, TexCoord);
uniform sampler2D CanTex;

uniform bool UseCanTexture = false;
...
vec4 pass1() {
    ...
    for (int i = 0; i < 3; i++)
        lighting += blinnPhong(Position, N, i);

    if (UseCanTexture) {
        vec3 texColor = texture(CanTex, TexCoord).rgb;
        return vec4(texColor * lighting, 1.0);
    } else {
...
FragColor = texture(CanTex, TexCoord);
